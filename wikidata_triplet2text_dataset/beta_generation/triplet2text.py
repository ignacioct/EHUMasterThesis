...
def main():
    ...
    for _, row in tqdm(
        ...
        with open("beta_gen_template_advanced.jinja2") as file_:
        ...
        # Second generation (rephrasing)
        user_prompt_rephrase = f"Now, rephrase the given sentence using different words and sentence structures, but keep all nouns exactly as they appear in the original sentence.\n\nOriginal sentence: {generated_text}"
        chat_input_rephrase = [
            {"role": "system", "content": rendered_prompt},
            {"role": "user", "content": user_prompt},
            {"role": "assistant", "content": generated_text},
            {"role": "user", "content": user_prompt_rephrase},
        ]

        outputs_rephrase = model.chat(
            chat_input_rephrase, sampling_params, use_tqdm=False
        )
        rephrased_text = outputs_rephrase[0].outputs[0].text
    ...
    output.to_csv("wikidata_triplet2text_beta_generated.csv", index=False)
